...
import { englishToSymbols, symbolsToEnglish, knowledgeMap } from '../utils/knowledgeTranslator'
  ...
  return (
    <>
      ...
      <div className="mb-8">
        ...
        {Object.entries(knowledgeMap).map(([char, symbol]) => (
              <button
                key={char}
                onClick={() => handleSymbolClick(symbol)}
                className="text-4xl p-2 border rounded-md hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 font-['Sigilos-Regular']"
              >
                {symbol}
              </button>
            ))}
      ...
      <div className="mb-8">
        ...
        <div className="flex flex-wrap gap-2 p-2 border border-green-600 min-h-[50px] mb-4 break-words font-['Sigilos-Regular'] text-2xl">
          {symbolText}
        </div>
      </div>
